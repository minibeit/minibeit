...
export const getWaitListApi = async (postId, doDate) => {
  return await withAuthInstance.get(
    GET_WAIT_LIST + postId + "/applicant/list?doDate=" + doDate
  );
};

export const getApproveListApi = async (postId, doDate) => {
  return await withAuthInstance.get(
    GET_APPROVE_LIST + postId + "/applicant/confirm/list?doDate=" + doDate
  );
};

export const approveOneApi = async (postId, postdoDateId, userId) => {
  return await withAuthInstance.post(
    APPROVE_ONE + postId + "/date/" + postdoDateId + "/apply/approve/" + userId
  );
};
export const cancelOneApi = async (postId, postdoDateId, userId) => {
  return await withAuthInstance.post(
    CANCEL_ONE +
      ...
      "/date/" +
      postdoDateId +
      "/apply/approve/cancel/" +
      userId
  );
};
export const rejectOneApi = async (
  postId,
  postdoDateId,
  userId,
  rejectValue
  ...
  return await withAuthInstance.post(
    REJECT_ONE + postId + "/date/" + postdoDateId + "/apply/reject/" + userId,
    data
  );
};
export const setAttendApi = async (postId, postdoDateId, userId, attend) => {
  ...
  return await withAuthInstance.post(
    SET_ATTEND + postId + "/date/" + postdoDateId + "/attend/change/" + userId,
    data
  );
};
