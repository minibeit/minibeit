= Minibeit API 문서 (Business Profile)
:doctype: book
:icons: font
:source-highlighter: highlightjs
:toc: left
:toclevels: 2
:sectlinks:
:snippets: ../../../build/generated-snippets

= Business Profile
---
== 기본 API

=== 비즈니스 프로필 생성

`POST /api/business/profile`

요청 HTTP
include::{snippets}/business-profile-create/request-parameters.adoc[]
include::{snippets}/business-profile-create/request-parts.adoc[]

성공 응답 HTTP
include::{snippets}/business-profile-create/http-response.adoc[]
include::{snippets}/business-profile-create/response-fields.adoc[]

=== 비즈니스 프로필 단건 조회

`GET /api/business/profile/{businessProfileId}`

요청 HTTP
include::{snippets}/business-profile-getOne/http-request.adoc[]
include::{snippets}/business-profile-getOne/path-parameters.adoc[]

성공 응답 HTTP
include::{snippets}/business-profile-getOne/http-response.adoc[]
include::{snippets}/business-profile-getOne/response-fields.adoc[]

=== 자신의 비즈니스 프로필 전체 조회

`GET /api/business/profile/list/{userId}`

요청 HTTP
include::{snippets}/business-profile-list-mine/http-request.adoc[]
include::{snippets}/business-profile-list-mine/path-parameters.adoc[]

성공 응답 HTTP
include::{snippets}/business-profile-list-mine/http-response.adoc[]
include::{snippets}/business-profile-list-mine/response-fields.adoc[]

=== 비즈니스 프로필 수정

`POST /api/business/profile/{businessProfileId}`

요청 HTTP
include::{snippets}/business-profile-update/path-parameters.adoc[]
include::{snippets}/business-profile-update/request-parameters.adoc[]
include::{snippets}/business-profile-update/request-parts.adoc[]

성공 응답 HTTP
include::{snippets}/business-profile-update/http-response.adoc[]
include::{snippets}/business-profile-update/response-fields.adoc[]

=== 비즈니스 프로필 삭제

`DELETE /api/business/profile/{businessProfileId}`

요청 HTTP
include::{snippets}/business-profile-delete/http-request.adoc[]
include::{snippets}/business-profile-delete/path-parameters.adoc[]

성공 응답 HTTP
include::{snippets}/business-profile-delete/http-response.adoc[]

=== 비즈니스 프로필 공유하기(초대)

`POST /api/business/profile/{businessProfileId}/share`

요청 HTTP
include::{snippets}/business-profile-share/http-request.adoc[]
include::{snippets}/business-profile-share/path-parameters.adoc[]
include::{snippets}/business-profile-share/request-fields.adoc[]

성공 응답 HTTP
include::{snippets}/business-profile-share/http-response.adoc[]


=== 비즈니스 프로필 공유 취소

`POST /api/business/profile/{businessProfileId}/expel`

요청 HTTP
include::{snippets}/business-profile-share-cancel/http-request.adoc[]
include::{snippets}/business-profile-share-cancel/path-parameters.adoc[]
include::{snippets}/business-profile-share-cancel/request-body.adoc[]

성공 응답 HTTP
include::{snippets}/business-profile-share-cancel/http-response.adoc[]

=== 비즈니스 프로필 권한 변경

`POST /api/business/profile/{businessProfileId}/change/{userId}`

요청 HTTP
include::{snippets}/business-profile-change-admin/http-request.adoc[]
include::{snippets}/business-profile-change-admin/path-parameters.adoc[]

성공 응답 HTTP
include::{snippets}/business-profile-change-admin/http-response.adoc[]


=== 비즈니스 프로필 게시물 리스트

`GET /api/business/profile/{businessProfileId}/posts`

요청 HTTP
include::{snippets}/business-profile-postList/http-request.adoc[]
include::{snippets}/business-profile-postList/path-parameters.adoc[]
include::{snippets}/business-profile-postList/request-parameters.adoc[]
성공 응답 HTTP
include::{snippets}/business-profile-postList/http-response.adoc[]
include::{snippets}/business-profile-postList/response-fields.adoc[]

== User 관련 API
=== 해당 비즈니스 프로필을 가지고 있는 유저 조회

`GET /api/user/list/business/profile/{businessProfileId}`

요청 HTTP
include::{snippets}/user-list-in-businessprofile/http-request.adoc[]
include::{snippets}/user-list-in-businessprofile/path-parameters.adoc[]

성공 응답 HTTP
include::{snippets}/user-list-in-businessprofile/http-response.adoc[]

== 비즈니스 프로필 후기 관련 API
=== 비즈니스 프로필 후기 작성하기

`POST /api/post/{postId}/review/{postDoDateId}`

요청 HTTP
include::{snippets}/business-review-create/http-request.adoc[]
include::{snippets}/business-review-create/path-parameters.adoc[]
include::{snippets}/business-review-create/request-fields.adoc[]

성공 응답 HTTP
include::{snippets}/business-review-create/http-response.adoc[]
include::{snippets}/business-review-create/response-fields.adoc[]

=== 비즈니스 프로필 후기 단건 조회

`GET /api/business/profile/review/{businessProfileReviewId}`

요청 HTTP
include::{snippets}/business-review-getOne/http-request.adoc[]
include::{snippets}/business-review-getOne/path-parameters.adoc[]

성공 응답 HTTP
include::{snippets}/business-review-getOne/http-response.adoc[]
include::{snippets}/business-review-getOne/response-fields.adoc[]

=== 비즈니스 프로필 후기 수정

`PUT /api/business/profile/review/{businessProfileReviewId}`

요청 HTTP
include::{snippets}/business-review-update/http-request.adoc[]
include::{snippets}/business-review-update/path-parameters.adoc[]
include::{snippets}/business-review-update/request-fields.adoc[]

성공 응답 HTTP
include::{snippets}/business-review-update/http-response.adoc[]
include::{snippets}/business-review-update/response-fields.adoc[]

=== 비즈니스 프로필 후기 삭제

`DELETE /api/business/profile/review/{businessProfileReviewId}`

요청 HTTP
include::{snippets}/business-review-deleteOne/http-request.adoc[]
include::{snippets}/business-review-deleteOne/path-parameters.adoc[]

성공 응답 HTTP
include::{snippets}/business-review-deleteOne/http-response.adoc[]

